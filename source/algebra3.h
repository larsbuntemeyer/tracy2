...
//
//	Modified by Paul Rademacher, 1998
//	- Added set, swap_row/swap_col functions
//
//	Modified by Aline N, 2009
//	- Add divide by zero length check to Length()
//
//	Modified by slsdo, 2010
//	- Added Cross Product operator
//	- Added rotation3Drad by Liming Zhao
//	- Added normalized to return a copy of a normalized vec3
//	- Added mat4*vec3
//  - Added mat4 determinant
...
public:
    ...
    vec3 normalized();					// return a normalized vec3
...
public:
    ...
    void set( float x, float y, float z, float a );	// set vector
...
public:
    ...
    void set(const vec3& v0, const vec3& v1, const vec3& v2);	// set matrix
...
mat3 rotation2Ddeg(const vec2& Center, const double angleDeg);	// rotation 2D
mat3 rotation3Drad(const vec3& axis, const float angleRad);	// rotation 3D
...
mat4 rotation3Ddeg(vec3 Axis, const double angleDeg);	// rotation 3D
...
inline vec3& vec3::operator /= ( const double d )
{
    double d_inv = 1./d; n[VX] *= d_inv; n[VY] *= d_inv; n[VZ] *= d_inv; return *this;
}
...
inline vec3 vec3::cross(vec3 &v) const
{
    vec3 tmp;
...
inline vec3 operator / (const vec3& a, const double d)
{
    double d_inv = 1./d; return vec3(a.n[VX]*d_inv, a.n[VY]*d_inv, a.n[VZ]*d_inv);
}
...
inline vec4& vec4::operator /= ( const double d )
{
    double d_inv = 1./d; n[VX] *= d_inv; n[VY] *= d_inv; n[VZ] *= d_inv; n[VW] *= d_inv; return *this;
}
...
inline void vec4::set( float x, float y, float z, float a )
{
...
inline vec4 operator / (const vec4& a, const double d)
{
    double d_inv = 1./d; return vec4(a.n[VX]*d_inv, a.n[VY]*d_inv, a.n[VZ]*d_inv, a.n[VW]*d_inv);
}
...
inline void mat3::set( const vec3& v0, const vec3& v1, const vec3& v2 )
{
    v[0] = v0; v[1] = v1; v[2] = v2;
}
...
inline void mat4::swap_rows( int i, int j )
{
    vec4 t;
...
inline void mat4::swap_cols( int i, int j )
{
    float t;
    int k;
...
inline double mat4::det() const
{
    ...
    return a[0][0] * (a[1][1] * (a[2][2] * a[3][3] - a[3][2] * a[2][3]) -
...
inline mat3 rotation2Ddeg(const vec2& Center, const double angleDeg)
{
...
inline mat3 rotation3Drad(const vec3& axis, const float angleRad)
{
...
inline mat4 rotation3Ddeg(vec3 Axis, const double angleDeg)
{
    double  angleRad = angleDeg * M_PI / 180.0,
            c = cos(angleRad),
            s = sin(angleRad),
